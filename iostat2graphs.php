...
if (is_uploaded_file($_FILES['csv_file']['tmp_name'])) {
    exec("perl iostat2graphs.pl {$csv_file} {$report_dir} {$width} {$height} {$requests_limit} {$bytes_limit} {$qlength_limit} {$wtime_limit} {$stime_limit} {$offset} {$duration} {$is_actual} 2>&1",
        $output, $return_var);
...
<html>
  <head>
    <title>Error - iostat2graphs</title>
  </head>
  <body>
    <p>
<?php echo $message; ?>
